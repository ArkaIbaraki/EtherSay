<div class="flex flex-col h-full">
    ...
    <div class="bg-white border-b px-4 md:px-6 py-3 md:py-4 flex items-center justify-between">
        <div class="flex items-center space-x-2 md:space-x-3 flex-1 min-w-0">
            ...
            <button 
                class="md:hidden p-1.5 hover:bg-gray-100 rounded-lg transition flex-shrink-0"
    ...
    <!-- Messages Area -->
    <div 
        class="flex-1 overflow-y-auto p-3 md:p-6 space-y-3 md:space-y-4 bg-gray-50"
    ...
    >
        @foreach($messages as $msg)
            ...
            @else
                <!-- Regular Message -->
                <div class="flex {{ $msg['sender_session_id'] === session()->getId() ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-[85%] sm:max-w-sm md:max-w-md {{ $msg['sender_session_id'] === session()->getId() ? 'order-2' : 'order-1' }}">
                        @if($msg['sender_session_id'] !== session()->getId())
                            <p class="text-xs text-gray-600 mb-1 ml-1">{{ $msg['sender_username'] }}</p>
                        @endif
                        
                        <div class="rounded-lg p-2.5 md:p-3 {{ $msg['sender_session_id'] === session()->getId() ? 'bg-blue-600 text-white' : 'bg-white text-gray-800' }} shadow">
                            @if($msg['message'])
                        ...
                        <p class="text-xs text-gray-500 mt-1 {{ $msg['sender_session_id'] === session()->getId() ? 'text-right' : 'text-left' }}">
    ...
    <div class="bg-white border-t p-2 md:p-4">
        ...
        <form wire:submit="sendMessage" class="flex items-end space-x-1 md:space-x-2">
            ...
            <div class="flex-1">
                ...
                @if($file)
                    <div class="mt-2 text-xs md:text-sm text-gray-600 flex items-center space-x-2 bg-blue-50 p-2 rounded">
                        <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        <span class="truncate max-w-[120px] sm:max-w-[200px] md:max-w-none flex-1">{{ $this->getFileName() }}</span>
                        <button type="button" wire:click="clearFile" class="text-red-600 hover:text-red-800 text-xl font-bold flex-shrink-0">
                            ×
                        </button>
                    </div>
            ...
            >
                <svg wire:loading.remove wire:target="sendMessage" class="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
                </svg>
                <svg wire:loading wire:target="sendMessage" class="animate-spin w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
            </button>
