        ...
        @forelse($onlineUsers as $index => $user)
            ...
            >
                <div class="relative flex-shrink-0">
                    <div class="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-sm md:text-base font-semibold">
                        {{ strtoupper(substr($user['username'], 0, 2)) }}
                    </div>
                    <div class="absolute bottom-0 right-0 w-2 h-2 md:w-3 md:h-3 bg-green-500 rounded-full border-2 border-white"></div>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="font-medium text-sm md:text-base text-gray-800 group-hover:text-blue-600 truncate">{{ $user['username'] }}</p>
